...
ConditionBuilder.prototype.buildFilterFn = function () {
  var filters = Object.keys(this._conditions).map(function (key) {
    ...
    switch (operator) {
    case 'BEGINS_WITH':
      return function(item) { return item && item[key].indexOf(values[0]) === 0 }
    case 'EQ':
      return function(item) { return item && item[key] == values[0] }
    case 'NE':
      return function(item) { return item && item[key] != values[0] }
    case 'LE':
      return function(item) { return item && item[key] <= values[0] }
    ...
    case 'GT':
      return function(item) { return item && item[key] > values[0] }
    case 'BETWEEN':
      return function(item) { return item && item[key] >= values[0] && item[key] <= values[1] }
    case 'IN':
      return function(item) { return item && values.indexOf(item[key]) != -1 }
    case 'NOT_CONTAINS':
      return function(item) { return item && item[key].indexOf(values[0]) == -1 }
    case 'CONTAINS':
      return function(item) { return item && item[key].indexOf(values[0]) != -1 }
    case 'NULL':
      return function(item) { return item && !item.hasOwnProperty(key) }
    case 'NOT_NULL':
      return function(item) { return item && item.hasOwnProperty(key) }
    default:
      throw new Error('Invalid comparison operator \'' + operator + '\'')
