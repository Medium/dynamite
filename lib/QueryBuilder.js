...
QueryBuilder.prototype.indexBetween = function (name, val1, val2) {
  if (typ.isNullish(val1)) throw new Error('Invalid lower bound for the range key: ' + val1)
  if (typ.isNullish(val2)) throw new Error('Invalid upper bound for the range key: ' + val2)
  this._rangeKeyName = name
  this._keyConditions.push({
    name: name,
    operator: 'BETWEEN',
