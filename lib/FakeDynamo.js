...
FakeTable.prototype.query = function(data) {
  ...
  var hash = data.HashKeyValue ||
             typeUtil.objectToValue(data.KeyConditions[this.primaryKey.hash.name].AttributeValueList[0])
  ...
  var keyConditionFn = (new ConditionBuilder(data.KeyConditions)).buildFilterFn()
  var queryFilterFn = (new ConditionBuilder(data.QueryFilter)).buildFilterFn()
  ...
  for (var field in data.AttributeUpdates) {
    ...
    if (update.Action === 'DELETE') {
      ...
      if (typeUtil.objectIsEmpty(update.Value)) {
        ...
        var type = typeUtil.objectToType(update.Value)
        var value = typeUtil.objectToValue(update.Value)
        if ((type === 'NS' || type === 'SS') && value.length > 0) {
          // remove from array
          for (var i = 0; i < value.length; i++) {
            var valueToDelete = value[i]
      ...
      if (!typeUtil.objectIsEmpty(update.Value)) {
        ...
        shouldSave = true
        item[field] = typeUtil.objectToValue(update.Value)
      }
      ...
      if (type === 'NS' || type === 'SS') {
        ...
        // increment a number
        item[field] = typ.isNullish(item[field]) ? value : item[field] + value
