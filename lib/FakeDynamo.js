...
FakeTable.prototype.query = function(data) {
  ...
  if (data.ExclusiveStartKey) {
    ...
    indexedKeyValues.splice(0, index + 1) // take off all the lesser range keys, and the start key
  }
  ...
  for (var i = 0; i < indexedKeyValues.length  && (limit === -1 || results.length < limit); i++) {
  ...
  if (data.Select === 'COUNT') {
    ...
    return Q.resolve({
      Items: typeUtil.packObjects(results, attributes),
      ConsumedCapacity: 1
    })
  }
}
...
FakeTable.prototype.scan = function(data) {
  ...
  var results = []
  for (var i = 0; i < sortedData.length && results.length < limit; i++) {
    if (exclusiveStartKey) {
      if (sortedData[i][hashKeyName] < exclusiveStartKey[hashKeyName] ||
          (sortedData[i][hashKeyName] === exclusiveStartKey[hashKeyName] &&
          sortedData[i][rangeKeyName] <= exclusiveStartKey[rangeKeyName])) {
        continue
      }
    }
  ...
  ret.Items = typeUtil.packObjects(results, attributes)
  // If there are more data to fetch, compose and return the LastEvaluatedKey attributes
  if (i < sortedData.length) {
    ...
    ret.LastEvaluatedKey = typeUtil.packObjects(sortedData[i - 1], primaryKeyAttribute)
  }
  return Q.resolve(ret)
}
...
FakeTable.prototype._getIndexedKeyValuesForHash = function(hash, attr) {
  ...
  for (var key in this.data[hash]) {
    keys.push(this.data[hash][key][attr])
  }
  return keys
}
