...
FakeTable.prototype.query = function(data) {
  ...
  if (limit !== -1 && results.length > limit) {
    result.LastEvaluatedKey = this._createLastEvaluatedKey(results[limit - 1])
  ...
  if (data.Select === 'COUNT') {
    ...
    result.Items = typeUtil.packObjects(results, attributes)
    return Q.resolve(result)
  }
}
...
FakeTable.prototype.scan = function(data) {
  ...
  if (i < sortedData.length) {
    ret.LastEvaluatedKey = this._createLastEvaluatedKey(sortedData[i - 1])
  }
  return Q.resolve(ret)
}
