...
/**
 * @param {Array.<FilterBuilder>} conditions An array of conditions, possibly null to indicate
 *     no conditions.
...
DynamoRequest.prototype.setKeyConditions = function (conditions) {
  ...
  if (conditions) {
    ...
    for (var i = 0; i < conditions.length; i += 1) {
      assert.ok(conditions[i] instanceof FilterBuilder, 'Expected FilterBuilder')
      conditions[i].populateObject(this.data.KeyConditions)
    }
  }
  return this
}
...
DynamoRequest.prototype.setQueryFilter = function (conditions) {
  ...
  if (conditions) {
    ...
    for (var i = 0; i < conditions.length; i += 1) {
      assert.ok(conditions[i] instanceof FilterBuilder, 'Expected FilterBuilder')
      conditions[i].populateObject(this.data.QueryFilter)
    }
  }
  return this
}
...
DynamoRequest.prototype.setScanFilter = function (conditions) {
  ...
  if (conditions) {
    ...
    for (var i = 0; i < conditions.length; i += 1) {
      assert.ok(conditions[i] instanceof FilterBuilder, 'Expected FilterBuilder')
      conditions[i].populateObject(this.data.ScanFilter)
    }
  }
  return this
}
